...
unsigned short TBit3[SITENUM]  = { 0, 0, 0, 0 };    // 0: Vref=0.8v
																				            // 1: Vref=0.6v
double IEN1H[SITENUM] = { 999.0 }, IEN2L[SITENUM] = { 999.0 }, REN1[SITENUM] = { 0.0f };
/* **************************************************************************************************** */
...
DUT_API int SupplyCurrent(short funcindex, LPCTSTR funclabel)	{
  ...
  rlyC.SetOn(-1);
    ...
    IINQ[site] = VinFPVI0.GetMeasResult(site, MIRET);
		// Offset
		//IINSD[site] += float(1.55e-6);//AA
		IINSD[site] += float(0e-6);//AB
		//IINQ[site] += float(26.4e-6);//AA
		IINQ[site] += float(0e-6);//AB
  ...
  // ****************************************** HSRON ******************************************
	...
	for(site = 0; site < SITENUM; site++)	{
		BEGIN_SINGLE_SITE(site);
			...
			// Offset
			HSRon[site] += float(-0.0303);// 30.3mohm
		END_SINGLE_SITE();
	}
  ...
  VinFPVI0.Set(FV, 12.0f, FPVI10_20V, FPVI10_1A, RELAY_ON, 5);
	...
	for(site = 0; site < SITENUM; site++)	{
      ...
      LSRon[site] = ( fabs( Load1mAVSW[site] - Load200mAVSW[site] ) / (fabs(Load200mAISW[site] - Load1mAISW[site]) + 0.1e-12f));
			// Offset
			LSRon[site] += float(-0.0323);// 32.3mohm
		END_SINGLE_SITE();
	}
...
DUT_API int CurrentLimit(short funcindex, LPCTSTR funclabel)	{
      ...
      END_SINGLE_SITE();
		...
		for (site = 0; site < SITENUM; site++)	{
			if (flag[site] == 0)	{
				...
				if (adresult[site] <= 1.0f && flag[site] == 0)	{
					Climit[site] = -StepV;
					flag[site] = 1;
				}
				else	{	
	...
	for (site = 0; site < SITENUM; site++)	{
		BEGIN_SINGLE_SITE(site);
	      ...
	      if (flag[site] == 0)	{
			...
			// Offset
			Ivalley->SetTestResult(site, 0, Climit0[site] + (1.75 * (Climit[site] - Climit0[site]))-0.04);
			// IVALLEY0
  ...
  for (StepV = 0.3f; StepV <= 2.0; StepV += 0.01f)	{
		SetV = 0;
    for (site = 0; site < SITENUM; site++)	{
			BEGIN_SINGLE_SITE(site);
				if (flag[site] == 0)	{
	...
	for(site = 0; site < SITENUM; site++ )
		INEG->SetTestResult(site, 0, Ineg[site]*1e3);
	// PowerDownReset
	PWR0();
	PWROFF();
	rlyC.SetOn(-1);
...
DUT_API int Switching1(short funcindex, LPCTSTR funclabel)	{
  ...
  rlyC.SetOn(-1);
	...
	rlyC.SetOn(CapVIN, /*CBSTSW,*/ VinFPVI, ENFOVI, FBEXT1K, ENQTMUa, -1);
	delay_ms(1);

	delay_ms(1);
	// VIN = 6
	VinFPVI0.Set(FV, 6.0f, FPVI10_20V, FPVI10_100MA, RELAY_ON, 10);
	delay_ms(10);
	// EN = 5
  ...
  qtmu0.SetInSource(QTMU_PLUS_SINGLE_SOURCE);
	...
	for(site = 0; site < SITENUM; site++)	{
		...
		Fsw[site] += 0.0588f;	// AB correlation, 58.8Khz
		// List
  ...
  qtmu0.SetInSource(QTMU_PLUS_SINGLE_SOURCE);
	...
	for(site = 0; site < SITENUM; site++ )	{
		...
		Toff[site] += -0.032f;	// AB correlation, offset -32ns
...
DUT_API int BangGap(short funcindex, LPCTSTR funclabel)	{
  ...
  rlyC.SetOn(-1);
	...
	FreshSiteFlagInit();
	// RELAY SETTINGS, FB-QTMUA w/o SWAP
	rlyC.SetOn(CapVIN, CBSTSW, VinFPVI, ENFOVI, FbFOVI, -1);
	delay_ms(1);
	// VIN = 6
	...
	delay_us(100);
	// FB 
	FB_FOVI.Set(FI, -1e-6, FOVI_2V, FOVI_100UA, RELAY_ON);
	delay_ms(5);

	FB_FOVI.MeasureVI(20, 10);
	for(site = 0; site < SITENUM; site++)	{
		vbg1[site] = FB_FOVI.GetMeasResult(site, MVRET);
		vbg1[site] += 0.00968f;	// AB correlation, offset 9.68mv
...
DUT_API int RegulatorV(short funcindex, LPCTSTR funclabel)	{
  ...
  FreshSiteFlagInit();
	...
	EnFOVI.Set(FV, 5.0f, FOVI_10V, FOVI_10MA, RELAY_ON);
	delay_us(300);
	FB_FOVI.Set(FI, -0.5e-6f, FOVI_1V, FOVI_10UA, RELAY_SENSE_ON); 
	delay_ms(1);
...
DUT_API int CurrentFB(short funcindex, LPCTSTR funclabel)	{
  ...
  FreshSiteFlagInit();
	...
	for(site=0; site<SITENUM; site++)	{
		ifb[site] = FB_FOVI.GetMeasResult(site, MIRET) * 1e9f;
...
DUT_API int SoftStartTime(short funcindex, LPCTSTR funclabel)	{
  ...
  // TODO: Add your function code here
	...
	VinFPVI0.Set(FV, 0.0f, FPVI10_5V, FPVI10_10MA, RELAY_ON);
	// SW
	SwFPVI1.Set(FI, -1e-3f, FPVI10_20V, FPVI10_1MA, RELAY_ON);
	// EN
	EnFOVI.Set(FV, 0.0f, FOVI_5V, FOVI_10MA, RELAY_ON);
	delay_ms(10);
	...
	delay_ms(10);
	// FB=1.25
	FB_FOVI.Set(FV, 1.25f, FOVI_10V, FOVI_10MA, RELAY_ON);
	delay_ms(1);
	// BST-SW 
	...
	qtmu0.Init();
	delay_ms(2);
	qtmu0.SetStartInput(QTMU_PLUS_IMPEDANCE_1M, QTMU_PLUS_VRNG_5V, QTMU_PLUS_FILTER_100KHz);
	qtmu0.SetStopInput(QTMU_PLUS_IMPEDANCE_1M, QTMU_PLUS_VRNG_5V, QTMU_PLUS_FILTER_100KHz);
	qtmu0.SetStartTrigger(4.8f, QTMU_PLUS_POS_SLOPE);
	qtmu0.SetStopTrigger(4.0f, QTMU_PLUS_POS_SLOPE);
  ...
  // TSS *****************************************************************
	for(site = 0; site < SITENUM; site++ )  {
	  qtmu0.SinglePlsMeas(site);
		Tss[site]=qtmu0.GetMeasureResult(site);
		// Offset
		Tss[site] += float(-350);//-0.35ms(350us) by correlation, offset
		TSS->SetTestResult(site, 0, fabs(Tss[site])*1e-3);
	}
	PWR0();
	PWROFF();
	rlyC.SetOn(-1);
...
DUT_API int FuncEN1(short funcindex, LPCTSTR funclabel)	{
  ...
  rlyC.SetOn(-1);
	...
	delay_ms(1);
	// EN
	EnFOVI.Set(FV, float(0.9), FOVI_5V,	FOVI_100MA, RELAY_ON);
 	delay_ms(10);
  ...
  for (vf = float(1.23); vf > float(0.9);) {
    ...
    IEN2L[site] = EnFOVI.GetMeasResult(site, MIRET, MAX_RESULT);
		...
		REN1[site] = 2.0f / IEN1H[site];
		// Offset
		REN1[site] += float(0.02e6);
	}
	// PowerDownReset
	PWR0();
	PWROFF();
...
DUT_API int FuncEN2(short funcindex, LPCTSTR funclabel)	{
  ...
  CParam *REN = StsGetParam(funcindex,"REN");
  //}}AFX_STS_PARAM_PROTOTYPES

  // TODO: Add your function code here
	for(site = 0; site < SITENUM; site++)	{
		REN		->SetTestResult(site, 0, REN1[site]*1e-6);
...
DUT_API int UVLO(short funcindex, LPCTSTR funclabel)	{
  ...
  CParam *UVLO_VHYS = StsGetParam(funcindex,"UVLO_VHYS");
	...
	double vf = 0.0f, StepV = 10e-3f, ii = 0.0f;
  ...
  rlyC.SetOn(-1);
	...
	// ************************************** VinUVLO ON **************************************
	// ************************************** VinUVLO ON **************************************
	for(vf = float(3.05); vf < float(4.55);)	{
		for(site = 0; site < SITENUM; site++)	{
			...
			if( (ICC[site] > float(800e-6f))&&flag[site]==0)	{			// 600uA(#2)
...
DUT_API int OVP(short funcindex, LPCTSTR funclabel)	{
  ...
  rlyC.SetOn(-1);
	...
	// ************************************** VinOVP ON **************************************
	// ************************************** VinOVP ON **************************************
	for(vf = float(17.0); vf < float(25.0);)	{//19->17, product spec.
		for(site = 0; site < SITENUM; site++)	{
			...
			if( (ICC[site] < float(700e-6f))&&flag[site]==0)	{
	...
	for(vf = float(21.0); vf > float(18.0);)	{
		for(site = 0; site < SITENUM; site++)	{
			...
			if( (ICC[site] > float(1000e-6f))&&flag[site]==0)	{
