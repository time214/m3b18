...
//#ifdef DEBUG
  ...
  delay_ms(1);
	...
	////for(StepV = -1.0f; StepV <= 0.0f; StepV = StepV + 0.1f )		// issue, FSW 260206-5
...
DUT_API int Switching1(short funcindex, LPCTSTR funclabel)	{
 ...
 //	delay_ms(1);
  ...
  rlyC.SetOn(-1);
	...
	//VinFPVI0.Set(FV, 6.5f, FPVI10_20V, FPVI10_100MA, RELAY_ON, 10);
...
DUT_API int SoftStartTime(short funcindex, LPCTSTR funclabel)	{
  ...
  // TODO: Add your function code here
	...
	qtmu0.SetStopTrigger(4.0f, QTMU_PLUS_POS_SLOPE);			// EN positive to SW positive(A-B)
  ...
  // TSS *****************************************************************
	for(site = 0; site < SITENUM; site++ )  {
	  ...
	  qtmu0.SinglePlsMeas(site);
		...
		Tss[site] += float(-0);//-0.35ms(350us) by correlation, offset
...
DUT_API int FuncEN1(short funcindex, LPCTSTR funclabel)	{
  ...
  rlyC.SetOn(-1);
    ...
    IEN2L[site] = EnFOVI.GetMeasResult(site, MIRET, MAX_RESULT);
		...
		REN1[site] += float(-0.04*1e6);// offset
	}
	// PowerDownReset
	PWR0();
	PWROFF();
